...
namespace Valve.VR
{
    public class SteamVR_LoadLevel : MonoBehaviour
    {
        ...
        public float postLoadSettleTime = 0.5f;

        // Time to fade loading screen in and out (also used for progress bar).
        public float loadingScreenFadeInTime = 0.5f;
        public float loadingScreenFadeOutTime = 0.5f;
        ...
        public void Trigger()
        {
            if (!loading && !string.IsNullOrEmpty(levelName)) {
                ...
                if (loading) Debug.Log("SteamVR_LoadLevel is loading. Could not start Coroutine.");
                else Debug.Log("LevelName is null or empty(?)");
            }

        }
        public void Trigger(string _name)
        {
            levelName = _name;
            if (!loading && !string.IsNullOrEmpty(levelName)) {
                StartCoroutine(LoadLevel());
        ...
        {
            var loader = FindObjectOfType<SteamVR_LoadLevel>();
                         //new GameObject("loader").AddComponent<SteamVR_LoadLevel>();
        ...
        IEnumerator LoadLevel()
        {
            ...
            if (!GetComponent<InteractionSystem.DontDestroyOnLoad>()) Destroy(gameObject);      // Added on 08.03.2022 by Miika Puljujärvi - Reason: We don't want to destroy this instance if it is attached to a gameObject with DontDestroyOnLoad.
            //Destroy(gameObject);
